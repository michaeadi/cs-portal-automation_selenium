package com.airtel.cs.commonutils.dataproviders.dataproviders;

import com.airtel.cs.commonutils.applicationutils.constants.ApplicationConstants;
import com.airtel.cs.commonutils.applicationutils.constants.CommonConstants;
import com.airtel.cs.commonutils.dataproviders.beantoexcel.ActionTagBeanToExcel;
import com.airtel.cs.commonutils.dataproviders.beantoexcel.ClientConfigExcelToBeanDao;
...
public class DataProviders extends Driver {
    ...
    public List<ClientConfigDataBean> getClientConfig() {
        ClientConfigExcelToBeanDao clientConfigExcelToBeanDao = new ClientConfigExcelToBeanDao();
        File excel = new File(excelPath);
        return clientConfigExcelToBeanDao.getData(excel.getAbsolutePath(), constants.getValue(ApplicationConstants.CLIENT_CONFIG));
    }

    public List<ClientConfigDataBean> getOpenApiClientConfig() {
        ClientConfigExcelToBeanDao clientConfigExcelToBeanDao = new ClientConfigExcelToBeanDao();
        File excel = new File(excelPath);
        return clientConfigExcelToBeanDao.getData(excel.getAbsolutePath(), constants.getValue(ApplicationConstants.OPEN_API_CLIENT_CONFIG));
    }

    @DataProvider(name = "FTRIssue")
    ...
    @DataProvider(name = "deActivateClientConfig")
    public Object[][] getDeActivateConfig() {
        File excel = new File(excelPath);
        ClientConfigExcelToBeanDao clientConfigExcelToBeanDao = new ClientConfigExcelToBeanDao();
        clientConfigDataBeans =
                clientConfigExcelToBeanDao.getData(excel.getAbsolutePath(), config.getProperty("ClientConfig"));
        List<ClientConfigDataBean> finalList = new ArrayList<>();

        for (ClientConfigDataBean clientConfig : clientConfigDataBeans) {
            if (clientConfig.getIsDeActivate() != null && clientConfig.getIsDeActivate().equalsIgnoreCase("yes"))
                finalList.add(clientConfig);
        }
        Object[][] hashMapObj = new Object[clientConfigDataBeans.size()][1];
        for (int i = 0; i < finalList.size(); i++) {
            hashMapObj[i][0] = finalList.get(i);
        }
        return hashMapObj;
