...
public class ServiceClassWidgetPOM extends BasePage {
    ...
    public String gettingServiceProfileNoResultFoundMessage() {
        UtilsMethods.printInfoLog("Validating error message when there is no data from com.airtel.cs.API : " + readText(serviceProfileNoResultFoundMessage));
        return readText(serviceProfileNoResultFoundMessage);
    }

    public boolean isServiceProfileNoResultFoundVisible() {
        UtilsMethods.printInfoLog("Validating error is visible when there is no data from com.airtel.cs.API : " + isElementVisible(serviceProfileNoResultFound));
        return isElementVisible(serviceProfileNoResultFound);
    }

    public boolean isServiceProfileErrorVisible() {
        UtilsMethods.printInfoLog("Validating error is visible when there is Error in com.airtel.cs.API : " + isElementVisible(serviceProfileError));
