...
public class AMTransactionsWidgetPOM extends BasePage {
    ...
    public String gettingAirtelMoneyNoResultFoundMessage() {
        UtilsMethods.printInfoLog("Validating error message when there is no data from com.airtel.cs.API : " + readText(airtelMoneyNoResultFoundMessage));
        return readText(airtelMoneyNoResultFoundMessage);
    }

    public boolean isAirtelMoneyNoResultFoundVisible() {
        UtilsMethods.printInfoLog("Validating error is visible when there is no data from com.airtel.cs.API : " + isElementVisible(airtelMoneyNoResultFound));
