...
public class DetailedUsageHistoryPOM extends BasePage {
    ...
    * */
    ...
    public Boolean isWidgetDisplay(){
        printInfoLog("Checking Detailed Usage history widget display: "+checkState(title));
        return checkState(title);
    }

    public Boolean isFreeCDR(){
        printInfoLog("Is Free CDR Checkbox displayed: "+checkState(freeCDR));
        return checkState(freeCDR);
    }

    public Boolean isTypeOfCDR(){
        printInfoLog("Is Type of CDR Option displayed: "+checkState(typeOfCDR));
        return checkState(typeOfCDR);
    }

    public Boolean isTodayDateFilter(){
        printInfoLog("Is Today Date Filter Option displayed: "+checkState(todayDateFilter));
        return checkState(todayDateFilter);
    }

    public Boolean isLast2DayDateFilter(){
        printInfoLog("Is last 2 day Date Filter Option displayed: "+checkState(last2DayDateFilter));
        return checkState(last2DayDateFilter);
    }

    public Boolean isLast7DayDateFilter(){
        printInfoLog("Is last 7 day Date Filter Option displayed: "+checkState(last7DayDateFilter));
        return checkState(last7DayDateFilter);
    }

    public Boolean isDatePickerDisplay(){
        printInfoLog("Is Date picker displayed: "+checkState(datePicker));
    ...
    public String getHeaders(int column){
        ...
        printInfoLog("Reading Header at POS("+column+"): "+readText(header));
    ...
    public Boolean isPagination(){
        printInfoLog("Is pagination displayed: "+checkState(pagination));
        return checkState(pagination);
    }

    public boolean getNoResultFound() {
        printInfoLog("Checking No Result Found Message Display");
