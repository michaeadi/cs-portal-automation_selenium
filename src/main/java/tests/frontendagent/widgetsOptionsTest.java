...
import com.airtel.cs.pagerepository.pagemethods.DetailedUsageHistoryPOM;
import com.airtel.cs.pagerepository.pagemethods.MoreAMTransactionsTabPOM;
import com.airtel.cs.pagerepository.pagemethods.MoreRechargeHistoryPOM;
        ...
        try {
            ...
            if (accountsBalanceAPI.getStatusCode() == 200) {
                ...
                for (int i = 0; i < size; i++) {
                    softAssert.assertEquals(daDetails.getDAId(i + 1), accountsBalanceAPI.getResult().get(i).getDaId(), "DA ID is not as received in com.airtel.cs.API on row " + i);
                    softAssert.assertEquals(daDetails.getDADescription(i + 1), accountsBalanceAPI.getResult().get(i).getDaDesc(), "DA Description is not as received in com.airtel.cs.API on row " + i);
                    softAssert.assertEquals(daDetails.getBundleType(i + 1).replace("-", "null") + " ", accountsBalanceAPI.getResult().get(i).getBundleType() + " ", "DA Bundle Type is not as received in com.airtel.cs.API on row " + i);
                    softAssert.assertEquals(daDetails.getDADateTime(i + 1), UtilsMethods.getDateFromEpochInUTC(accountsBalanceAPI.getResult().get(i).getExpiryDate(), "dd-MMM-yyyy"), "DA Date Time is not as received in com.airtel.cs.API on row " + i);
                    softAssert.assertEquals(daDetails.getDABalance(i + 1), accountsBalanceAPI.getResult().get(i).getCurrentDaBalance(), "DA Current Balance is not as received in com.airtel.cs.API on row " + i);
                    if (i != 0) {
    ...
    public void accumulatorDetailsTest(HeaderDataBean Data) {
        ...
        try {
            ...
            if (accumulatorAPI.getStatusCode() == 200) {
                ...
                for (int i = 0; i < size; i++) {
                    softAssert.assertEquals(daDetails.getValueCorrespondingToAccumulator(i + 1, 1).trim(), accumulatorAPI.getResult().get(i).getId(), "Accumulator ID is not as received in com.airtel.cs.API on row " + i);
                    softAssert.assertEquals(daDetails.getValueCorrespondingToAccumulator(i + 1, 2).trim(), String.valueOf(accumulatorAPI.getResult().get(i).getValue()), "Accumulator Value is not as received in com.airtel.cs.API on row " + i);
                ...
                if (amTransactionHistoryAPI.getStatusCode() != 200) {
                    ...
                    if (count > 0) {
                        ...
                        for (int i = 0; i < count; i++) {
                            ...
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 2), UtilsMethods.getDateFromEpoch(new Long(amTransactionHistoryAPI.getResult().getData().get(i).getTransactionDate()), config.getProperty("AMHistoryTimeFormat")), i + "th Date is not expected as com.airtel.cs.API response.");
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 3), amTransactionHistoryAPI.getResult().getData().get(i).getService(), i + "th Service name is not expected as com.airtel.cs.API response.");
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 4), amTransactionHistoryAPI.getResult().getData().get(i).getSource(), i + "th Sender MSISDN is not expected as com.airtel.cs.API response.");
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 5), amTransactionHistoryAPI.getResult().getData().get(i).getMsisdn(), i + "th Receiver MSISDN is not expected as com.airtel.cs.API response.");
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 6), amTransactionHistoryAPI.getResult().getData().get(i).getSecondPartyName(), i + "th Beneficiary name is not expected as com.airtel.cs.API response.");
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 7), amTransactionHistoryAPI.getResult().getData().get(i).getTransactionId(), i + "th Transaction Id is not expected as com.airtel.cs.API response.");
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 8), amTransactionHistoryAPI.getResult().getData().get(i).getServiceCharge(), i + "th Service Charge is not expected as com.airtel.cs.API response.");
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 9), amTransactionHistoryAPI.getResult().getData().get(i).getBalanceBefore(), i + "th Pre-balance is not expected as com.airtel.cs.API response.");
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 10), amTransactionHistoryAPI.getResult().getData().get(i).getBalanceAfter(), i + "th Post-balance is not expected as com.airtel.cs.API response.");
                            softAssert.assertEquals(amTransactionsWidget.getValueCorrespondingToHeader(i + 1, 11), amTransactionHistoryAPI.getResult().getData().get(i).getStatus(), i + "th Status is not expected as com.airtel.cs.API response.");
