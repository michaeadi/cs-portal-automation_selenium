...
public class CRBTWidgetTest extends BaseTest {
    ...
    public void validateMyTuneTab(HeaderDataBean data) {
        ...
        if (!searchTune.getStatusCode().equalsIgnoreCase("200")) {
            softAssert.fail("com.airtel.cs.API Response "+searchTune.getMessage());
...
//            softAssert.assertEquals(crbtWidget.getSearchHeader(4).trim().toLowerCase(), data.getRow4().trim().toLowerCase(), "Header name for Search tab not same as expected at POS-4");
            ...
            for (int i = 0; i < size; i++) {
                softAssert.assertEquals(crbtWidget.getValueSearch(i+1,1).trim().toLowerCase(),(searchTune.getResult().get(i).getName()+searchTune.getResult().get(i).getSinger()).toLowerCase().trim(),"Name is not same as expected in com.airtel.cs.API response");
                softAssert.assertEquals(crbtWidget.getValueSearch(i+1,2).trim(),searchTune.getResult().get(i).getRenewalAmount().trim(),"Renewal Amount is not same as expected in com.airtel.cs.API response");
                softAssert.assertEquals(crbtWidget.getValueSearch(i+1,3).trim(),searchTune.getResult().get(i).getPeriod().trim(),"Period is not same as expected in com.airtel.cs.API response");
            }
        } else {
...
//            softAssert.assertEquals(crbtWidget.getTop20Header(4).trim().toLowerCase(), data.getRow4().trim().toLowerCase(), "Header name for Search tab not same as expected at POS-4");
            ...
            for (int i = 0; i < size; i++) {
                softAssert.assertEquals(crbtWidget.getValueTop20(i+1,1).trim().toLowerCase(),(top20Tune.getResult().get(i).getName()+top20Tune.getResult().get(i).getSinger()).toLowerCase().trim(),"Name is not same as expected in com.airtel.cs.API response");
                softAssert.assertEquals(crbtWidget.getValueTop20(i+1,2).trim(),top20Tune.getResult().get(i).getRenewalAmount().trim(),"Renewal Amount is not same as expected in com.airtel.cs.API response");
                softAssert.assertEquals(crbtWidget.getValueTop20(i+1,3).trim(),top20Tune.getResult().get(i).getPeriod().trim(),"Period is not same as expected in com.airtel.cs.API response");
            }
        } else {
            softAssert.assertTrue(crbtWidget.isNoResultMessage(), "No Result message does not in case of no ringtone found with search keyword.");
            softAssert.assertTrue(crbtWidget.isNoResultImg(), "No Result Image does not in case of no ringtone found with search keyword.");
