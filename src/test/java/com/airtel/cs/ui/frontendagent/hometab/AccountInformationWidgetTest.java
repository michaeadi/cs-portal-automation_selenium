...
public class AccountInformationWidgetTest extends Driver {
    ...
    public void verifyUnbilledAmount() {
        try {
            ...
            assertCheck.append(actions.assertEqualStringType(pages.getAccountInformationWidget().getCurrentMonthUnBillAmountForCalls(), pages.getAccountInformationWidget().getValue(postpaidAccountInformation, "currentMonthUnbilledAmountForCalls", "unbilledCalls"), "Current month un-billed amount for calls displays as expected", "Current month un-billed amount for calls not displays as expected"));
            assertCheck.append(actions.assertEqualStringType(pages.getAccountInformationWidget().getCurrentMonthUnBillAmountForSms(), pages.getAccountInformationWidget().getValue(postpaidAccountInformation, "currentMonthUnbilledAmountForSms", "unbilledSMS"), "Current month un-billed amount for sms displays  as expected", "Current month un-billed amount for sms not displays as expected"));
    ...
    public void verifyAccountInfoDetailedIcon() {
        ...
        try {
            ...
            assertCheck.append(actions.assertEqualStringType(pages.getAccountInformationWidget().getDueDate().toLowerCase(), pages.getAccountInformationWidget().getValue(postpaidAccountInformation, "dueDate", "dueDate").toLowerCase(), "Due date displays as expected", "Due date not displays as expected"));
            assertCheck.append(actions.assertEqualStringType(pages.getAccountInformationWidget().getCurrentCycle().toLowerCase(), pages.getAccountInformationWidget().getValue(postpaidAccountInformation, "currentCycle", "currentCycle").toLowerCase(), "Current cycle displays as expected", "Current cycle not displays as expected"));
            ...
            assertCheck.append(actions.assertEqualStringType(pages.getAccountInformationWidget().getLastMonthBillDate().toLowerCase(), pages.getAccountInformationWidget().getValue(postpaidAccountInformation, "dueDate", "dueDate").toLowerCase(), "Last month bill date displays as expected", "Last month bill date not displays as expected"));
            ...
            if (StringUtils.isEmpty(tempCreditLimitAPI)) {
                ...
                if (tempCreditLimit > 0) {
                    ...
                    assertCheck.append(actions.assertEqualStringType(validTillDate.toLowerCase(), pages.getAccountInformationWidget().getCurrentCycleEndDate().toLowerCase(), "Validate till date is same as Current Cycle End date as expected", "Validate till date is not same as Current Cycle End date as expected"));
                } else {
