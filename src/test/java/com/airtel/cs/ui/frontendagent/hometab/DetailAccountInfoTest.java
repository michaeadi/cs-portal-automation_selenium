...
public class DetailAccountInfoTest extends Driver {
    ...
    public void connectionTypeAndUMPermissionTest() {
        ...
        try {
            ...
            if (connectionType.equalsIgnoreCase("POSTPAID") && umAccountInformationPermission && umViewBillPermission) {
                assertCheck.append(actions.assertEqual_boolean(pages.getAccountInformationWidget().isAccountInformationWidgetDisplay(), true, "View bill is visible", "View bill not visible"));
            } else {
    ...
    public void accountInfoIcon(){
        try{
            ...
            assertCheck.append(actions.assertEqual_boolean(pages.getDetailAccountInfoWidget().isActionIconVisibleOnAccountInfo(), true, "Detailed account information icon visible", "Detailed account information icon not visible"));
            pages.getDetailAccountInfoWidget().openAccountInformationDetailPage();
            assertCheck.append(actions.assertEqual_stringType(pages.getDetailAccountInfoWidget().getAccountInfoDetailWidget().toUpperCase(), "ACCOUNT INFORMATION DETAIL", "Account Information Detail display as expected in detailed account info", "Account Information Detail not display as expected in detailed account info"));
            actions.assertAllFoundFailedAssert(assertCheck);
        }catch (NoSuchElementException | TimeoutException | NullPointerException e) {
