...
public class RechargeHistoryMenuWidgetTest extends Driver {
    ...
    public void rechargeHistoryMenuTest(HeaderDataBean data) {
        try {
            ...
            assertCheck.append(actions.assertEqualIntType(statusCode, 200, "Recharge History API status code matched and is: " + statusCode, "Recharge History API status code NOT matched and is: " + statusCode, false));
            if (statusCode == 200) {
                ...
                if (rechargeHistoryAPI.getResult().size() == 0 || rechargeHistoryAPI.getResult() == null) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getMoreRechargeHistoryPage().getValueCorrespondingToRechargeHeader(i + 1, 2).replaceAll("[\\s\\-()]", " "), rechargeHistoryAPI.getResult().get(i).getDateTime(), "Date & Time received is as expected on row " + i, "Date & Time received is not as expected on row " + i));
                        ...
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getMoreRechargeHistoryPage().getValueCorrespondingToRechargeHeader(i + 1, 8).replaceAll("[\\s\\-()]", " "), ((rechargeHistoryAPI.getResult().get(i).getExpiryDate() == null) ? "-" : rechargeHistoryAPI.getResult().get(i).getExpiryDate()), "Expiry date received is as expected on row " + i, "Expiry date received is not as expected on row " + i));
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getMoreRechargeHistoryPage().getValueCorrespondingToRechargeHeader(i + 1, 9).replaceAll("[\\s\\-()]", " "), ((rechargeHistoryAPI.getResult().get(i).getOldExpiryDate() == null) ? "-" : rechargeHistoryAPI.getResult().get(i).getOldExpiryDate()), "Old Expiry date received is as expected on row " + i, "Old Expiry date received is not as expected on row " + i));
