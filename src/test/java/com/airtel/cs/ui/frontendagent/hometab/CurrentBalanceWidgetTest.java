...
import com.airtel.cs.model.cs.response.plans.PlansResult;
...
public class CurrentBalanceWidgetTest extends Driver {
    ...
    public void currentBalanceWidgetHeaderTest() {
        try {
            ...
            assertCheck.append(actions.assertEqualStringType(pages.getCurrentBalanceWidgetPage().getFooterAuuidYCP(), loginAUUID, "Auuid shown at the footer of the Your Current Plan widget and is correct", "Auuid NOT shown at the footer of Your Current Plan widget"));
            assertCheck.append(actions.assertEqualStringType(pages.getCurrentBalanceWidgetPage().getMiddleAuuidTYP(), loginAUUID, "Auuid shown at the middle of the Your Current Plan widget and is correct", "Auuid NOT shown at the middle of Your Current Plan widget"));
            assertCheck.append(actions.assertEqualStringType(pages.getCurrentBalanceWidgetPage().getCurrentPlanHeaderText(), "YOUR CURRENT PLAN", "Current Plan Widget Header Text Matched", "Current Plan Widget Header Text NOT Matched"));
            actions.assertAllFoundFailedAssert(assertCheck);
        } catch (Exception e) {
    ...
    public void mainAccountBalanceTest() {
        try {
            ...
            if (result.getMainAccountBalance() != null) {
                ...
                assertCheck.append(actions.assertEqualStringType(pages.getCurrentBalanceWidgetPage().gettingCurrentBalanceCurrency(), result.getMainAccountBalance().getCurrency(), "Current Balance Currency is as Received in API", "Current Balance Currency is not as Received in API"));
            } else {
                commonLib.warning("Unable to get Main Balance from API");
            }
            if (result.getLastRecharge() != null) {
                try {
                    assertCheck.append(actions.assertEqualStringType(pages.getCurrentBalanceWidgetPage().gettingLastRechargeAmount(), result.getLastRecharge().getAmount(), "Last Recharge is as Received in API", "Last Recharge is not as Received in API"));
                ...
                assertCheck.append(actions.assertEqualStringType(pages.getCurrentBalanceWidgetPage().getLastRechargeDateTime(), "- -", "Last Recharge Date & Time is as expected", "Last Recharge Date & Time is not as expected"));
            }
            actions.assertAllFoundFailedAssert(assertCheck);
        } catch (Exception e) {
    ...
    public void voiceDataSmsTest() {
        try {
            ...
            if (result.getData() != null) {
                ...
                assertCheck.append(actions.assertEqualStringType(currentBalanceWidgetPage.getDataBalance().replace("-", "null"), result.getData().getBalance(), "Data Balance is as Received in API", "Data Balance is not as Received in API"));
                assertCheck.append(actions.assertEqualStringType(currentBalanceWidgetPage.getDataExpiryDate(), UtilsMethods.getDateFromEpoch(result.getData().getExpireTime(), constants.getValue(CommonConstants.BALANCE_EXPIRY_PATTERN)), "Data Expiry Date is as Received in API", "Data Expiry Date is not as Received in API"));
            }
