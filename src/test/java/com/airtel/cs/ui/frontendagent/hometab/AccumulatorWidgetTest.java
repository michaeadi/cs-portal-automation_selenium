            ...
            selUtils.addTestcaseDescription("Validating Accumulator Details of User :" + customerNumber+",Validate accumulator widget header display as per config,Validate accumulator row data must be displayed as per api response.", "description");
                ...
                if (statusCode == 200) {
                    ...
                    for (int i = 0; i < size; i++) {
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getDaDetailsPage().getValueCorrespondingToAccumulator(i + 1, 1).trim(), accumulatorAPI.getResult().get(i).getId(), "Accumulator ID as received in API on row " + i,"Accumulator ID is not as received in API on row " + i));
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getDaDetailsPage().getValueCorrespondingToAccumulator(i + 1, 2).trim(), String.valueOf(accumulatorAPI.getResult().get(i).getValue()), "Accumulator Value as received in API on row " + i,"Accumulator Value is not as received in API on row " + i));
