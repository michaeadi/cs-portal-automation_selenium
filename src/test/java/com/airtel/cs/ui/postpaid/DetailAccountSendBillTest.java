...
public class DetailAccountSendBillTest extends Driver {
   ...
   public void validateSendBill() {
       try {
           ...
           if (statusCode == 500)
               assertCheck.append(actions.assertEqualBoolean(pages.getDetailAccountSendBill().isErrorMessageVisible(), true, "Unable to Fetch Data error message is visible", "Unable to Fetch Data error message is NOT visible"));
           else {
               ...
               for (int row = 0; row < size; row++) {
                   ...
                   if ("INVOICE".equalsIgnoreCase(transactionType)) {
                       ...
                       assertCheck.append(actions.assertEqualStringType(pages.getDetailAccountSendBill().getEmailId(), EmailID, "Email ID is same as expected", "Email ID is NOT same as expected"));
                       ...
                       /**
                        Again clicking on send bill icon after closing the Issue detail pop up
                        */
                       pages.getDetailAccountSendBill().clickOnSendBill(row);
                       pages.getDetailAccountSendBill().selectReason();
                       pages.getDetailAccountSendBill().enterComment("Automation Testing");
    ...
    public void checkActionTrail() {
        try {
            ...
            assertCheck.append(actions.assertEqualStringType(pages.getDetailAccountSendBill().getComment(), "Automation Testing", "Comment for send bill is expected", "Comment for send bill is not as expected"));
