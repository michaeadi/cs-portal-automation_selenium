...
public class SmartCashTransactionHistoryTest extends Driver {
    ...
    @Test(priority = 2, groups = {"ProdTest", "SmokeTest", "SanityTest"}, dataProvider = "HeaderData", dataProviderClass = DataProviders.class, dependsOnMethods = {"openCustomerInteraction"})
    ...
    @DataProviders.Table(name = "SmartCash Transaction History")
    @Test(priority = 3, groups = {"ProdTest", "SanityTest"}, dependsOnMethods = {"transactionHistoryWidgetLayoutTest", "openCustomerInteraction"})
            ...
            if (statusCode != 200) {
                ...
                if (count > 0) {
                    for (int i = 0; i < count; i++) {
                        ...
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getSmartCashTransactionHistory().getValueCorrespondingToHeader(i + 1, 6), amTransactionHistoryAPI.getResult().getData().get(i).getSecondPartyName(), (i + 1) + "th Beneficiary name is expected as API response.", (i + 1) + "th Beneficiary name is not expected as API response."));
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getSmartCashTransactionHistory().getValueCorrespondingToHeader(i + 1, 7), amTransactionHistoryAPI.getResult().getData().get(i).getTransactionId(), (i + 1) + "th Transaction Id is expected as API response.", (i + 1) + "th Transaction Id is not expected as API response."));
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getSmartCashTransactionHistory().getValueCorrespondingToHeader(i + 1, 8), amTransactionHistoryAPI.getResult().getData().get(i).getTxnChannel(), (i + 1) + "th Transaction Channel is expected as API response.", (i + 1) + "th Transaction Channel is not expected as API response."));
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getSmartCashTransactionHistory().getValueCorrespondingToHeader(i + 1, 9), amTransactionHistoryAPI.getResult().getData().get(i).getServiceCharge(), (i + 1) + "th Service Charge is expected as API response.", (i + 1) + "th Service Charge is not expected as API response."));
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getSmartCashTransactionHistory().getValueCorrespondingToHeader(i + 1, 10), amTransactionHistoryAPI.getResult().getData().get(i).getBalanceBefore(), (i + 1) + "th Pre-balance is expected as API response.", (i + 1) + "th Pre-balance is not expected as API response."));
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getSmartCashTransactionHistory().getValueCorrespondingToHeader(i + 1, 11), amTransactionHistoryAPI.getResult().getData().get(i).getBalanceAfter(), (i + 1) + "th Post-balance is expected as API response.", (i + 1) + "th Post-balance is not expected as API response."));
                        assertCheck.append(actions.matchUiAndAPIResponse(pages.getSmartCashTransactionHistory().getValueCorrespondingToHeader(i + 1, 12), amTransactionHistoryAPI.getResult().getData().get(i).getStatus(), (i + 1) + "th Status is expected as API response.", (i + 1) + "th Status is not expected as API response."));
