...
public class WalletInformationTest extends Driver {
    ...
    public void testWalletInformationWidgetData() {
        try {
            ...
            assertCheck.append(actions.matchUiAndAPIResponse(pages.getWalletInformation().getBarringStatus(), clmDetails.getResult().getDetails().get(0).getWallets().get(0).getStatus() ,"Barring status is same as Expected", "Barring status  is not same as Expected"));
